...
export const ChatHeader = ({ webSocketModeRef = {} }) => {
    ...
    return (
        <div className={`top-0 z-10 flex justify-center items-center h-12 ${selectedConversation?.messages?.length === 0 ? 'bg-none' : 'bg-gradient-to-r from-blue-600 to-blue-700 sticky'}  py-2 px-4 text-sm text-white dark:border-none dark:bg-black dark:text-neutral-200`}>
            {
                selectedConversation?.messages?.length > 0 ? 
                <div className={`absolute top-6 left-1/2 transform -translate-x-1/2 -translate-y-1/2`}>
                    <span className="text-lg font-semibold text-white">🔍 数据洞察助手</span>
                </div> 
                :
                null
            } 
                        ...
                        <label className="flex items-center gap-2 cursor-pointer flex-shrink-0">
                            ...
                            <div
                                ...
                                className={`relative inline-flex h-5 w-10 items-center cursor-pointer rounded-full transition-colors duration-300 ease-in-out ${
                                    chatHistory ? 'bg-blue-600 dark:bg-blue-500' : 'bg-gray-200'
                                }`}
                            >
                    ...
                    <div className="flex items-center gap-2 whitespace-nowrap">
                        <label className="flex items-center gap-2 cursor-pointer flex-shrink-0">
                            ...
                            <div
                                ...
                                className={`relative inline-flex h-5 w-10 items-center cursor-pointer rounded-full transition-colors duration-300 ease-in-out ${
                                    webSocketModeRef.current ? 'bg-blue-600 dark:bg-blue-500' : 'bg-gray-200'
                                }`}
                            >
                                <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform duration-300 ease-in-out ${
